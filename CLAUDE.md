...
- **Never interpolate or re-serialize a rejected xero-node call.** Generated methods reject with a JSON *string* holding the request headers; through 13.4.0 that meant `Authorization: Bearer …`, and the token-endpoint rejects carried the Basic client-secret header and refresh-token body (Xero security notice, Sept 2026; fixed in 13.5.0). Render thrown values through `formatError()` / `ensureError()`, which parse the blob via `src/helpers/xero-api-error.ts` and emit only the status and Xero's own message. `npm run verify:deps` asserts both the installed library and those helpers keep a credential out.
