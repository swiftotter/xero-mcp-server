...
export async function wasRecentlyCreatedByCurrentUser(
  ...
  try {
    ...
    // The creation note is either the bare base string or, when a purpose
    // was supplied at create time, the base followed by " (purpose)". Match
    // the base as a prefix so the purpose suffix doesn't defeat the check.
    const base = auditNoteDetails("Created");
